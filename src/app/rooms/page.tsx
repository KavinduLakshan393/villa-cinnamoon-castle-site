...
export default function RoomsPage() {
  return <main id="main">
    ...
    <section className="section section--paper"><div className="container"><div className="section-heading reveal"><div><p className="eyebrow">Room by room</p><h2>Five individually photographed bedrooms.</h2></div><p className="lede">The rooms use neutral labels because the owner has not yet approved names or mapped each photograph to a specific bed size.</p></div><div className="room-grid">{roomData.map((room) => <article className="card room-card reveal" key={room.label}><div className={`card-media${room.portrait ? " card-media--portrait" : ""}`}><ResponsivePicture image={room.image} sizes="(max-width: 860px) 100vw, 50vw" /></div><div className="card-body"><span className="tag">{room.tag}</span><p className="card-meta">{room.label}</p><h3>{room.title}</h3><p>{room.copy}</p></div></article>)}</div></div></section>
    <section className="section"><div className="container"><div className="story-grid"><div className="image-frame image-frame--portrait reveal"><ResponsivePicture image={bathroom} sizes="(max-width: 860px) 100vw, 50vw" /></div><div className="story-copy reveal" id="bathrooms"><p className="eyebrow">Two full bathrooms</p><h2>Hot water is available.</h2><p className="lede">The property record confirms two full bathrooms with hot water. The supplied media set includes one bathroom photograph showing a wall-mounted hot-water shower unit.</p><div className="notice">The website does not imply that every bathroom looks identical. Contact the host for any room-specific questions.</div></div></div></div></section>
    <section className="section section--sand"><div className="container"><div className="section-heading reveal"><div><p className="eyebrow">At a glance</p><h2>Accommodation facts.</h2></div><p className="lede">These are the confirmed totals for the entire home.</p></div><div className="feature-list reveal">{facts.map(([title, copy]) => <div className="feature-row" key={title}><strong>{title}</strong><span>{copy}</span></div>)}</div><div className="button-row" style={{ marginTop: "2rem" }}><Link className="button" href="/contact/">Ask about your dates</Link><Link className="button button--quiet" href="/gallery/">View all photographs</Link></div></div></section>
  </main>;
}
