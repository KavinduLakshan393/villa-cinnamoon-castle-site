...
export type NormalizedInquiry = {
  ...
  checkOut: string;
  guests: number;
  inquiryType: "availability" | "pricing" | "long-stay" | "general";
  message: string;
...
export function normalizeInquiry(input: InquiryInput = {}): NormalizedInquiry {
  const preferredContact = ["email", "phone", "messaging"].includes(String(input.preferredContact))
    ? input.preferredContact as NormalizedInquiry["preferredContact"]
    : "email";
  const inquiryType = ["availability", "pricing", "long-stay", "general"].includes(String(input.inquiryType))
    ? input.inquiryType as NormalizedInquiry["inquiryType"]
    : "availability";
  ...
  return {
    ...
    guests: Number.parseInt(String(input.guests), 10),
    inquiryType,
